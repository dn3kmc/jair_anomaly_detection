...
for f in listdir(mypath):
	...
	for score_number in range(0,num_scores):
		for gaussian_window_size in gaussian_window_sizes:
			if "ts_object" in f:
				...
				joblib.dump(roc_auc_dict, "prophet_roc_auc_all/" + roc_auc_dict_name)
	...
	if roc_auc_dict_list:
		...
		plt.title('Prophet ROC for ' + ts.name[:-4])
		plt.legend(loc="lower right")
		plt.tight_layout()
		plt.show()

		# save the one with the best auc
		joblib.dump(best_roc_auc_dict, "prophet_roc_auc_best/" + roc_auc_dict_name)
