...
for f in listdir(mypath):
		...
		for gaussian_window_size in gaussian_window_sizes:
			if "ts_object" in f:
				...
				if ts.get_length() >= 400:
					...
					name = ts.name
					result_dict = joblib.load("../jair_work_step_three_anomaly_detection/htm_scores/htm_scores_" + str(score_number) + "_" + ts.name + "_dict")
					...
					roc_auc_dict = {"TS Name": ts.name, "FPRS": fpr, "TPRS": tpr, "Thresholds": thresholds, "AUC": roc_auc, "Dict Name": "htm_roc_auc_" + str(score_number) + "_" + ts.name + "_dict"}
					roc_auc_dict_name = roc_auc_dict["Dict Name"]
					joblib.dump(roc_auc_dict, "htm_roc_auc_all/" + roc_auc_dict_name)
	...
	if roc_auc_dict_list:
		...
		plt.title('HTM ROC for ' + ts.name[:-4])
		plt.legend(loc="lower right")
		plt.tight_layout()
		plt.show()

		# save the one with the best auc
		joblib.dump(best_roc_auc_dict, "htm_roc_auc_best/" + roc_auc_dict_name)
