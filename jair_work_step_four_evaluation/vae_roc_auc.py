...
for f in listdir(mypath):
	...
	for score_number in range(0,num_scores):
		for gaussian_window_size in gaussian_window_sizes:
			if "ts_object" in f:
				...
				roc_auc_dict = {"TS Name": ts.name, "FPRS": fpr, "TPRS": tpr, "Thresholds": thresholds, "AUC": roc_auc, "Dict Name": "vae_roc_auc_" + str(score_number) + "_gaussian_window_" + str(gaussian_window_size) + "_" + name + "_window_size_" + str(window_size) + "_mcmc_iteration_" + str(mcmc_iteration) + "_latent_dim_" + str(latent_dim)}
				roc_auc_dict_name = roc_auc_dict["Dict Name"]
				joblib.dump(roc_auc_dict, "vae_roc_auc_all/" + roc_auc_dict_name)
	...
	if roc_auc_dict_list:
		...
		plt.title('VAE ROC for ' + ts.name[:-4])
		plt.legend(loc="lower right")
		plt.tight_layout()
		plt.show()

		# save the one with the best auc
		joblib.dump(best_roc_auc_dict, "vae_roc_auc_best/" + roc_auc_dict_name)
