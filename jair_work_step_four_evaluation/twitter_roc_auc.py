...
for f in listdir(mypath):
			...
			if "ts_object" in f:
				...
				if not ts.miss:
					if ts.period > 1:
						...
						name = ts.name
						result_dict = joblib.load("../jair_work_step_three_anomaly_detection/twitter_scores/twitter_scores_" + str(score_number) + "_" + name + "_max_anoms_" + str(max_anoms) + "_direction_" + str(direction) + "_alpha_" + str(alpha))
						...
						roc_auc_dict = {"TS Name": ts.name, "FPRS": fpr, "TPRS": tpr, "Thresholds": thresholds, "AUC": roc_auc, "Dict Name": "twitter_roc_auc_" + str(score_number) + "_" + name + "_max_anoms_" + str(max_anoms) + "_direction_" + str(direction) + "_alpha_" + str(alpha)}
						roc_auc_dict_name = roc_auc_dict["Dict Name"]
						joblib.dump(roc_auc_dict, "twitter_roc_auc_all/" + roc_auc_dict_name)
	...
	if roc_auc_dict_list:
		...
		plt.title('Twitter ROC for ' + ts.name[:-4])
		plt.legend(loc="lower right")
		plt.tight_layout()
		plt.show()

		# save the one with the best auc
		joblib.dump(best_roc_auc_dict, "twitter_roc_auc_best/" + roc_auc_dict_name)
